...
public class StringExample {
	...
	public static void main(String[] args) {
		...
		int idx = subject.indexOf("그래밍");   //"그"의 위치를 찾음
		System.out.println(idx);
		//문자열 못 찾았을 때
		idx = subject.indexOf("java");
		System.out.println(idx);   // -1을 반환
		...
		String newStr = oldStr.replace("자바", "java"); //(바꾸기 전, 바꾼 후)
		System.out.println(newStr);
		
		//문자열 자르기 - substring
		//매개변수에 따라서 자르는 방법 다름
		//1) 매개변수가 1개 일 경우 - 해당 위치로부터 뒤로 다 자름
		String ssn3 = "123456-1234567";
		String firstNum = ssn3.substring(7);
		System.out.println(firstNum);
		//2) 매개변수가 2개 일 경우 - 시작 위치 ~ 끝나는 위치 앞까지 자름
